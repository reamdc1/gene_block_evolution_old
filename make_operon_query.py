...
def parser_code():
    ...
    parser.add_argument("-o", "--outfile", dest="outfile", metavar="FILE", default='./operon_query.fa',
                help="Resulting BLAST query file the is derived from an operon file and refrence organism(s).")
    ...
    #################################################################################################
    # EDIT 10/29/2014:  																			#
    # The default behavior I am looking for is that E. coli K12 MG1655 is used as the default, and 	#
    # single reference.  The user may set the 'r' flag to change this single reference.				#
    # Conversly, if the 'R' option is set, then all organisms will be considered. 					#
    # If the -R is supplied with a value, then a user supplied accession number file is provided	#
...
# the needs of the programmer. The defaulted variables allow a single operon to be chosen individually.  The program
# will also store teh results of this function in a folder titled blast_query_files, in the recovery folder.
def make_operon_fasta2(gene_list, genbank_list, num_processors, outfile, ref_list):
    ...
    pool = Pool(processes = num_processors)
    organism_dict_for_recovery = dict(pool.map(parallel_operon_fasta, refrence_file_path_list))
